...
\item Transceiver의 대폭적인 전력 감소(37.8\%p)가 switch의 증가분(12.7\%p)을 크게 상회하여, 전체적으로 약 25\% 순 절감을 달성합니다. 이는 transceiver를 제거하는 것이 아니라 "더 효율적인 형태로 switch에 통합"하는 전략입니다.

\item CPO는 transceiver를 제거하여 큰 전력 절감을 달성하지만, 그 기능을 스위치 내부로 이동시키므로 스위치 자체의 전력 소비는 증가합니다. 이는 "전력을 없애는" 것이 아니라 "더 효율적인 방식으로 재배치"하는 것입니다.
...
\textbf{1. 핵심 개념 정의}
\begin{itemize}
\item\textbf{Radix}: Switch가 제공하는 port 수이며 High radix는 많은 수의 port를 의미합니다 (예: 128포트 이상).
\item\textbf{Shuffle}: 여러 switch의 port를 상호 연결하여 하나의 논리적 대형 switch처럼 작동하도록 만드는 재배치 작업.
\end{itemize}
...
\textbf{A. Traditional 방식 (Non-CPO High Radix):}
\begin{itemize}
...
\textbf{B. CPO 방식 (Internal Shuffle):}
\begin{itemize}
...
\textbf{4. Twin-Port의 내부 구조(1.6T twin-port transceiver)}
...
\textbf{World Size}: 단일 scale-up domain 내에서 직접 연결될 수 있는 GPU의 총 개수. 
...
\textbf{1. SerDes (Serializer/Deserializer)}:
\begin{itemize}[itemsep=1pt,parsep=0pt,topsep=2pt]
...
\textbf{2. Long-Reach (LR) SerDes vs Short-Reach (SR) SerDes}:
\vspace{2mm}
...
\textbf{3. 왜 LR SerDes가 더 많은 전력을 쓰는가?}
\begin{itemize}[itemsep=1pt,parsep=0pt,topsep=2pt]
...
\textbf{4. PCB (Printed Circuit Board) Trace}:
\begin{itemize}[itemsep=1pt,parsep=0pt,topsep=2pt]
...
\begin{tcolorbox}[breakable,colback=blue!10!white,colframe=blue!40!black,title=\textbf{핵심 인사이트: DSP - Public Enemy Number One}]
\setlength{\parskip}{2pt}
...
\begin{tcolorbox}[breakable,colback=blue!10!white,colframe=blue!40!black,title=\textbf{핵심 인사이트: Figure of Merit (FoM) - Interconnect의 성적표}]
\setlength{\parskip}{2pt}
...
\begin{tcolorbox}[breakable,colback=blue!10!white,colframe=blue!40!black,title=\textbf{핵심 인사이트: I/O Wall - AI 시대의 새로운 Bottleneck}]
\setlength{\parskip}{2pt}
...
\begin{tcolorbox}[breakable,colback=blue!5!white,colframe=blue!75!black,title=\textbf{기술 용어: Wide I/O vs SerDes - Paradigm Shift}]
\setlength{\parskip}{2pt}
